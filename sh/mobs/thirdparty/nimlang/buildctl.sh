...
SRCDIR=$g_srcdir
TARBALL=$g_srcdir/nim-0.14.2-libc.so.6.tar.gz
ROOT=/scratch/mobs/nimlang/nim-0.14.2
...
unittest_cmd() {
    local cmd
    cmd="$g_installbuild_dir/nim/bin/nim -h"
    $cmd
    local cmd="$g_installbuild_dir/nim/bin/nim -v"
    $cmd
}
